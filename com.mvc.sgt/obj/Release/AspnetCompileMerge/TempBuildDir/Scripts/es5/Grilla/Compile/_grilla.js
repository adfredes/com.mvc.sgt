...
(function () {
    ...
    function renderListaReservas() {
        ...
        var btnCantidadSesionesModal_click = function (e) {
            ...
            options.sesionesReservadas.forEach(function (s, i) {
                return s.sesiones.forEach(function (se) {
                    var _sesion = {
                        "AgendaID": 1,
                        "TurnoID": null,
                        "Numero": i + 1,
    ...
    function setContextMenu() {
        ...
        function CancelarReserva(celda) {
            var divReserva = celda.children[0];
            if (divReserva.dataset.turnoid > 0) {
                var url = Domain + 'Sesion/Reserva/Delete';
                var param = {};
                param.turnoID = divReserva.dataset.turnoid;
                var promise = ajaxPromise('DELETE', url, param);
                promise.then(function (data) { return dibujarGrilla(); }, function (data) {
                ...
                options.sesionesReservadas = options.sesionesReservadas.filter(function (sesion) {
                    return sesion.fecha != id_1.fecha || sesion.hora != id_1.hora
        ...
        function contextMenuClick(key, opt, e) {
            switch (key) {
                ...
                case 'anularSesion':
                    ...
                    break;
                case 'cambiarTurno':
                    var sesionView = options.divGrilla.querySelector('#ctrlEditSesionGrilla');
                    var dSesionID = sesionView.querySelector("#dSesionID");
                    if (dSesionID) {
                    ...
                    break;
                case 'datosPaciente':
                    var pacienteView = options.divGrilla.querySelector('#ctrlPacienteViewAgenda');
                    var hPacienteID = pacienteView.querySelector("#hPacienteID");
                    if (hPacienteID) {
    ...
    function renderSesiones() {
        ...
        _sesiones.forEach(function (mValue) {
            return mValue.sesiones = options.sesiones.filter(function (fValue) {
                return mValue.TurnoID == fValue.TurnoID && mValue.Numero == fValue.Numero
...
//# sourceMappingURL=_grilla.js.map